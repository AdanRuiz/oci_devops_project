...
function getStateHistory() { return screen.getByTestId('state-history'); }
function getWorkLogs()     { return screen.getByTestId('work-logs'); }
function getSidebar()      { return screen.getByTestId('task-details-sidebar'); }
...
describe('R3 - Completed task: minimum required information', () => {
    test('displays the task name (title)', () => {
        ...
        expect(
            screen.getByText('Implement REST API endpoint')
        ).toBeInTheDocument();
    });
...
describe('Mock function - onBack spy', () => {
    test('calls onBack exactly once when the Back button is clicked', async () => {
        ...
        render(
            <TaskDetailView task={TASK} history={[]} logs={[]} onBack={onBack} />
        );
        await act(async () => {
