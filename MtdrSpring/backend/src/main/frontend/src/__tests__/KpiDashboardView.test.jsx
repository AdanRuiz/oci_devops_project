...
function getSprintTotalsCard() { return screen.getByTestId('sprint-totals-card'); }
function getYourStatsCard()    { return screen.getByTestId('your-stats-card'); }
...
describe('R7 - Per-person KPIs: tasks and hours for each developer per sprint', () => {
    ...
    test('shows personal stats for a different user (bob) in Your Stats', () => {
        render(
            <KpiDashboardView
                {...BASE_PROPS}
                currentUserEmail="bob.jones@oracle.com"
            />
        );
        const statsCard = getYourStatsCard();
