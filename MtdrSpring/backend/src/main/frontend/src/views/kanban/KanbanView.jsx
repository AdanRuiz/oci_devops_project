import { Box, Card, CardContent, CircularProgress, FormControl, MenuItem, Select, Typography } from '@mui/material';
...
function PriorityBadge({ priority }) {
    ...
    return (
        <Box sx={{ display: 'inline-flex', px: '8px', py: '2px', borderRadius: '20px', bgcolor: style.bgcolor }}>
    ...
    return (
        ...
        >
            <CardContent sx={{ p: '14px 16px !important' }}>
                ...
                <Typography sx={{ fontSize: '0.875rem', fontWeight: 700, color: '#1A1A1A', mb: '6px', lineHeight: 1.4 }}>
                    {task.title}
                </Typography>
    ...
    return (
        <Box sx={{ mb: '32px' }}>
            ...
            <Typography sx={{ fontWeight: 700, fontSize: '0.95rem', color: '#1A1A1A', mb: '12px' }}>
                {userName}{' '}
                <Typography component="span" sx={{ fontWeight: 400, color: '#9E9E9E', fontSize: '0.95rem' }}>
                    ({totalTasks})
                </Typography>
            </Typography>
...
export default function KanbanView({ projectName, sprints = [], sprintId = '', users = [], tasks = [], isLoading, onSprintChange, onTaskSelect }) {
    return (
        <Box>
                ...
                <>
                    ...
                    {users.map(user => {
                        ...
                        const userTasks = tasks.filter(t =>
                            (t.assignee?.id ?? t.assigneeId) === userId
                        );
                        return (
                            <UserRow
                                key={userId}
