...
function getMemberEmail(m) { return m.user?.email ?? m.email ?? ''; }
function getMemberId(m)    { return m.user?.id    ?? m.id; }
    ...
    return (
        <>
            <Card sx={{ border: '1px solid #E8E8E8', borderRadius: '8px', boxShadow: 'none', bgcolor: '#ffffff' }}>
                <CardContent sx={{ p: '14px 16px !important', display: 'flex', alignItems: 'center', gap: '14px' }}>
                    {/* Square avatar */}
                    <Box
                        sx={{
                            width: 40,
            ...
            >
                ...
                <DialogContent>
                    <Typography sx={{ fontSize: '0.875rem', color: '#717171' }}>
                        <strong style={{ color: '#1A1A1A' }}>{displayName}</strong> will be removed from the project and lose access immediately.
                    </Typography>
                </DialogContent>
                <DialogActions sx={{ px: '24px', pb: '16px', gap: '8px' }}>
                    ...
                    <Button
                        variant="contained"
                        disabled={isRemoving}
                        onClick={() => { onRemove(userId); setConfirmOpen(false); }}
...
export default function ProfileView({
    ...
    return (
        <Box>
            ...
            <Card sx={{ border: '1px solid #E8E8E8', borderRadius: '8px', boxShadow: 'none', bgcolor: '#ffffff', mb: '24px' }}>
                <CardContent sx={{ p: '28px !important' }}>
                    <Box sx={{ display: 'flex', alignItems: 'flex-start', gap: '40px' }}>
                        ...
                        <Box sx={{ display: 'flex', flexDirection: 'column', gap: '14px', flexShrink: 0 }}>
                            ...
                            >
                                <Typography sx={{ color: '#ffffff', fontWeight: 700, fontSize: '2.4rem', letterSpacing: '0.04em' }}>
                                    {initials}
                                </Typography>
                            </Box>
                            <Box>
